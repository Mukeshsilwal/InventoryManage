...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.csrf().disable()
                .authorizeRequests()
                .antMatchers("/api4/","/csv/**","/csv1/**","/auth/login").permitAll()
                .antMatchers("/api3/").permitAll()
                .antMatchers("/api3/user/{id}").hasAuthority("ADMIN")
                .antMatchers("/api3/{id}").hasAuthority("ADMIN")
                .antMatchers("/api4/{id}").hasAuthority("ADMIN")
                .antMatchers("/role/**").hasAuthority("ADMIN")
                .antMatchers("/auth/create_user").hasAuthority("Admin")
